...
class ToolbarInserts extends HTMLElement {
    ...
    link = {
        ...
        click(link) {
            ...
            windowManager.openWindow("/sysc/scripts/components/form/mat-editor/dialog/link-dialog", options).then((matWindow) => {
    ...
    image = {
        ...
        click(event) {
            ...
            windowManager.openWindow("/sysc/scripts/components/form/mat-editor/dialog/image-upload-dialog", options).then((matWindow) => {
                matWindow.addEventListener("ok", (event) => {
                    let value = event.target.contents.value;
                    document.getSelection().removeAllRanges();
                    document.getSelection().addRange(rangeAt);
    ...
    text = {
        ...
        click(event) {
            ...
            windowManager.openWindow("/sysc/scripts/components/form/mat-editor/dialog/text-dialog", options).then((matWindow) => {
                matWindow.addEventListener("ok", (event) => {
                    let value = event.target.contents.value;
                    document.getSelection().removeAllRanges();
                    document.getSelection().addRange(rangeAt);
    ...
    static get template() {
        return loader("sysc/scripts/components/form/mat-editor/toolbar/toolbar-inserts.html")
    }
